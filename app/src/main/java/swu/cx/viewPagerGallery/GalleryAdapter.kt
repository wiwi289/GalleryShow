...
class PhotoViewHolder(itemView: View) :RecyclerView.ViewHolder(itemView){
    ...
    fun bindWithPhotoItem(photoItem:PhotoItem){
        with(itemView){
            shimmerGallery.apply {
                setShimmerColor(0x55FFFFFF)
                setShimmerAngle(0)
                startShimmerAnimation()
            }
            textUser.text = photoItem.user
            textLikes.text = photoItem.likes.toString()
            textFavourites.text = photoItem.favorites.toString()
            imageView.layoutParams.height = photoItem.webformatHeight
        }
        Glide.with(itemView)
            .load(photoItem.largeImageURL)
            ...
            .listener(object :RequestListener<Drawable>{
                ...
                override fun onResourceReady(
                    ...
                    return false.also {itemView.shimmerGallery?.stopShimmerAnimation() }
                }
